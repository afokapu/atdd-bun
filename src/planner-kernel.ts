...
export type PlanKind = "wagon" | "feature" | "wmbt" | "acceptance" | "train" | "interlocking" | "contract";
...
const typedKinds: Record<string, PlanKind> = { wagon: "wagon", feature: "feature", wmbt: "wmbt", acc: "acceptance", train: "train", interlocking: "interlocking", contract: "contract" };
...
function typedRefs(value: unknown): string[] {
  if (typeof value === "string") return [...value.matchAll(/\b(?:wagon|feature|wmbt|acc|train|interlocking|contract):[a-z][a-z0-9-]*(?::[A-Za-z0-9._-]+)*/g)].map(match => match[0]);
...
function structuralRefs(artifact: PlanArtifact): string[] {
  ...
  const add = (value: unknown) => { if (typeof value === "string" && /^(?:wagon|feature|wmbt|acc|train|interlocking|contract):/.test(value)) out.push(value); };
 ...
 * wagon members, interlocking participants/routes, and WMBT acceptance ownership. */
