...
import { validateTelemetryPlan } from "../../src/telemetry-plan.ts";
...
const violations = (await Promise.all(roots.map(validateTelemetryPlan))).flat().map(item => ({ ...item, line: 1, col: 1, source_line: "" }));
