...
import { validatePlannerSchemas } from "../../src/planner-schema-validator.ts";
...
const violations = (await Promise.all(roots.map(validatePlannerSchemas))).flat().map(item => ({ ...item, line: 1, col: 1, source_line: "" }));
