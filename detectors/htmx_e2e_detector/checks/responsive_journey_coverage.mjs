...
await runCheck("responsive-journey-coverage", (root, specs, plan, report) => {
  ...
  for (const j of plan.journeys) if (j.exposed && j.frontend && !covered.has(j.id)) report("tester.htmx.responsive-journey-coverage", j.path, 1, `exposed journey ${j.id} has no RESP browser spec bound by \`// Journey: ${j.id}\`; nothing proves its screens work from mobile to desktop`);
});
