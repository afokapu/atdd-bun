...
await runCheck("exposed-journey-e2e-coverage", (root, specs, plan, report) => {
  ...
  for (const j of plan.journeys) if (j.exposed && j.frontend && !covered.has(j.id)) report("tester.htmx.exposed-journey-e2e-coverage", j.path, 1, `exposed journey ${j.id} has no E2E or SMOKE browser spec bound by \`// Journey: ${j.id}\``);
});
