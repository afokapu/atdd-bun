...
await runCheck("e2e-spec-naming", (root, specs, plan, report) => {
  for (const s of specs) if (s.playwright && !s.e2eNamed) report("tester.htmx.e2e-spec-naming", s.file, 1, "imports @playwright/test but is not named *.e2e.ts; bun test collects *.test.* and *.spec.* files and would run it with the wrong runner", firstLine(s.text));
});
