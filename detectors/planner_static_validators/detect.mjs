...
import { validateStaticPlannerConventions } from "../../src/planner-validators.ts";
...
const violations = (await Promise.all(roots.map(validateStaticPlannerConventions))).flat().map(item => ({ ...item, line: 1, col: 1, source_line: "" }));
