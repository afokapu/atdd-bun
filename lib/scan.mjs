...
/** Whether `path` lies under an excluded directory: a whole segment (`node_modules`) or a whole run of
 * segments (`.claude/worktrees`). Unlike a substring test it never excludes `src/builder.ts` for `build`. */
export function isExcludedPath(path, excludes) {
  const segments = path.split(/[\\/]/);
  return excludes.some((ex) => {
    const run = ex.split("/").filter(Boolean);
    return run.length > 0 && segments.some((_, at) => run.every((part, offset) => segments[at + offset] === part));
  });
}
